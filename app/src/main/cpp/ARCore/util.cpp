...
static GLuint LoadShader(GLenum shader_type, const char* shader_source) {
  ...
  if (!compiled) {
    ...
    LOGE("arcore::util::Could not compile shader %d:\n%s\n", shader_type,
         buf);
    free(buf);
...
GLuint CreateProgram(const char* vertex_shader_file_name,
  ...
  if (program) {
    ...
    CheckGlError("arcore::util::glAttachShader");
    ...
    if (link_status != GL_TRUE) {
      ...
      if (buf_length) {
        ...
        if (buf) {
          ...
          LOGE("arcore::util::Could not link program:\n%s\n", buf);
          free(buf);
        }
      }
...
bool LoadPngFromAssetManager(int target, const std::string& path) {
  ...
  static struct JNIData {
    ...
    constexpr char kHelperClassName[] =
        "com/example/dimension/JniInterface";
    ...
    LOGE("arcore::util::Could not find Java helper class %s",
         kHelperClassName);
    return {};
  }();
